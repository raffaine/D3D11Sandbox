...
void BasicScene::CreateShaders()
{
	...
	hr = m_device->CreateVertexShader(bytes, bytesRead, nullptr, m_pVShader.ReleaseAndGetAddressOf());
	hr = m_device->CreateInputLayout(iaDesc, _countof(iaDesc), bytes, bytesRead, m_pVSInputLayout.ReleaseAndGetAddressOf());
	delete[] bytes;
	...
	hr = m_device->CreatePixelShader(bytes, bytesRead, nullptr, m_pPShader.ReleaseAndGetAddressOf());
	delete[] bytes;

	// Create Constant Buffers
	CD3D11_BUFFER_DESC cbdsc{ sizeof(ConstantBufferStruct), D3D11_BIND_CONSTANT_BUFFER };
	hr = m_device->CreateBuffer(&cbdsc, nullptr, m_pConstantBuffer.ReleaseAndGetAddressOf());


	if (FAILED(hr)) {
		WCHAR buff[256] = {};
		swprintf_s(buff, L"Basic Scene failed to load Shader Data \n");
...
void BasicScene::CreateMesh()
{
	...
	hr = m_device->CreateBuffer(&vbdsc, &vData, m_pVertexBuffer.ReleaseAndGetAddressOf());
	...
	hr = m_device->CreateBuffer(&ibdsc, &iData, m_pIndexBuffer.ReleaseAndGetAddressOf());

	if (FAILED(hr)) {
		WCHAR buff[256] = {};
		swprintf_s(buff, L"Basic Scene failed to load Mesh Data \n");
		OutputDebugStringW(buff);
		return;
	}
}
...
void BasicScene::CreateViewAndPerspective() {
	...
	XMStoreFloat4x4(&m_cbData.View, XMMatrixTranspose(
	...
	XMStoreFloat4x4(&m_cbData.Projection, XMMatrixTranspose(
...
void BasicScene::Update() {
	...
	XMStoreFloat4x4(&m_cbData.World, XMMatrixTranspose(
...
void BasicScene::Render() {
	...
	m_context->UpdateSubresource(m_pConstantBuffer.Get(), 0, nullptr, &m_cbData, 0, 0);
